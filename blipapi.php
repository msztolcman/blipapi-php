 ...
 * @version 0.02.5
 * @version $Id: blipapi.php 18 2007-12-26 15:33:56Z mysz $
...
class BlipApi {
	...
	/**
 	 ...
 	 * @var string
 	 */
	protected $_uagent		= 'BlipApi/0.1 (http://blipapi.googlecode.com)';
	...
	/**
 	 ...
 	 *
 	 * As key need have to be mime type of reply, and for value - function name to call, or 2 values array: 0 - object, 1 - method to call.
 	 *
 	 * @access protected
 	 * @var array
 	 */
	...
	/**
 	 ...
 	 *
 	 * For specified keys, call proper __set_* method. Throws InvalidArgumentException exception when incorrect key was specified.
 	 *
	...
	/**
 	 ...
 	 *
 	 * Param $headers have to be an array, where key is header name, and value - header value, or string in 'Header-Name: Value'.
	...
	protected function _parse_headers ($headers) {
		if (!$headers) {
			...
			throw new UnexpectedValueException (sprintf ('%s::$headers have to be an array or string, but %s given.', __CLASS__, gettype ($headers)), -1);
		}
	...
	protected function __query ($url, $method='get', $data=null, $opts=array ()) {
		...
		# jeśli któraś z metod __query__* zwróciła dodatkowe nagłówki do wysłania, to miało to byc jednorazowe, czyli teraz usuwamy je z zestawu
	...
	protected function __parse_reply ($reply) {
		...
		if (isset ($this->_parsers[$this->_format])) {
			...
			if ($formatter &&
 			   	(
					(is_array ($formatter) && isset ($formatter[1]) && is_object ($formatter[0]) && method_exists ($formatter[0], $formatter[1]))
					||
	...
	/**
 	 * Reading update
 	 *
 	 * It's hard to explain what are doing specified parameters. Please consult with offcial API documentation: {@link http://www.blip.pl/api-0.02.html}.
 	 *
	...
	protected function _cmd__subscription_read ($user=null, $include=array(), $direction = 'both') {
		...
		if (!in_array ($direction, array ('both', 'to', 'from'))) {
			throw new UnexpectedValueException (sprintf ('Incorrect param: "direction": "%s". Allowed values: both, from, to.', $direction), -1);
		}
